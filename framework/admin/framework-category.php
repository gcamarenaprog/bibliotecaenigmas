...
/*-----------------------------------------------------------------------------------*/
function tie_cat_options ( $value = false ){
	$data = false;
	
	$tie_cats_options = get_option( 'tie_cats_options' );

	if( ! empty( $tie_cats_options[ $value['cat'] ] ) ){
		$category_data = $tie_cats_options[ $value['cat'] ];
	}

	if( ! empty( $category_data[ $value['id'] ] ) ){
		$data = $category_data[$value['id']];
	}
...
function tie_category_fields( $tag = false ) {
	...
	<td colspan="2">
		...
		<div class="tiepanel-item">
			...
			<?php
				...
				tie_cat_options( array(
					"name"    => __( 'Custom Sidebar', 'tie' ),
					"id"      => "cat_sidebar",
					"type"    => "select",
					"cat"     => $t_id ,
...
function tie_save_extra_category_fileds( $term_id = false ) {
